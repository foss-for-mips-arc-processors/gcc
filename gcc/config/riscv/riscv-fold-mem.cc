...
public:
  ...
  {
    return TARGET_64BIT && riscv_fold_mem;
  }
  ...
  for (insn = get_insns (); insn; insn = NEXT_INSN (insn))
	...
	continue;
      ...
      rtx src = (GET_CODE(SET_SRC(set)) == SIGN_EXTEND) ? 
		       XEXP(SET_SRC(set), 0) : SET_SRC(set);
      rtx dest = (GET_CODE(SET_DEST(set)) == SIGN_EXTEND) ? 
		       XEXP(SET_DEST(set), 0) : SET_DEST(set);
	...
	continue;
      ...
      HOST_WIDE_INT scaled_offset = (unsigned HOST_WIDE_INT) 
	      cumulative_inc << shift_amount;
      ...
      for (rtx_insn *curr = NEXT_INSN (insn_addr); curr && curr != insn; curr = NEXT_INSN (curr)) {
          if (!NONDEBUG_INSN_P (curr)) continue;
          subrtx_iterator::array_type array;
          FOR_EACH_SUBRTX (iter, array, PATTERN (curr), NONCONST) {
              const_rtx x = *iter;
              if (MEM_P (x)) {
                  /* If we find any other access that coflicts with the picked store/load.  */
                  if (true_dependence (mem, GET_MODE (mem), x) || 
                      canon_output_dependence (mem, false, x, GET_MODE (x), curr)) {
                      hazard = true; break;
                  }
              }
          }
	      ...
	      if (DF_REG_DEF_COUNT (REGNO (src)) == 1)
		{
		  ...
		  if (def_ref && !DF_REF_CHAIN (def_ref))
		      ...
		      if (def_set && REG_P (SET_SRC (def_set)))
			{
			  ...
			  if (validate_change (insn, &SET_SRC (set), orig_src, 0))
			    df_insn_rescan (insn);
			}
		    }
		}
