...
;; values (in range -2048 to 2047). Avoid materializing the const and fuse
;; into the add (with an additional add for 2nd value). Makes a 3 insn
;; sequence into 2 insn.
;;
;; Do not match SP/FP bases: after PR120811, LRA/addptr emits a ordered
;; addi pair for large frame offsets that fold-mem-offsets can turn into
;; ld/sd with a simm12.  Allowing this pattern on SP/FP lets late-combine
;; squash that pair back into a single out-of-range PLUS.
