/* Verify that the RHX-100 scheduler attributes every instruction of a
   ...
   Group under test:
     A (unfused ALU op) + A (unfused ALU op) + A (unfused ALU op) + A (unfused ALU op)
...
/* { dg-final { scan-rtl-dump-times {;;\s+1--> b\s+\d+: i\s+\d+ \S+=\S+\^0x[0-9a-f]+} 2 "sched2" } } */
/* { dg-final { scan-rtl-dump-times {;;\s+2--> b\s+\d+: i\s+\d+ \S+=\S+\^0x[0-9a-f]+} 2 "sched2" } } */
