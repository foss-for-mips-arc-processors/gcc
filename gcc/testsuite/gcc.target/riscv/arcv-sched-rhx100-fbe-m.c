/* Verify that a fused ALU macro-op does not block the memory pipe: a fused
   ...
   Group under test:
     FBE (fused zero-extract (shift-pair) ALU+ALU macro-op) + M (unfused load)
   ...
   The fused pair is pure ALU, so it must take slot A and leave slot B for the
...
f_fbe_m (void)
{
  ...
  asm volatile ("" : "=r"(u0), "=r"(p1));
  unsigned t0 = u0 << 8; unsigned r0 = t0 >> 4;
  int r1 = p1[128];
  asm volatile ("" :: "r"(r0), "r"(r1) : "memory");
}

/* { dg-final { scan-rtl-dump {;;\s+1--> b\s+\d+: i\s+\d+ \S+=\S+<<0x8[^\n]*\n;;\s+1--> b\s+\d+: i\s+\d+ \S+=\S+ 0>>0x4} "sched2" } } */
