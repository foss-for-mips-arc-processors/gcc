/* Verify that the RHX-100 scheduler does not end a cycle in the middle of
   ...
   Group under test:
     FMA (fused multiply-add ALU pair) + FMA (fused multiply-add ALU pair)
...
f_fma_fma (void)
{
  ...
  asm volatile ("" : "=r"(x0), "=r"(y0), "=r"(z0), "=r"(x1), "=r"(y1), "=r"(z1));
  int m0 = x0 * y0; int r0 = m0 + z0;
  int m1 = x1 * y1; int r1 = m1 + z1;
  asm volatile ("" :: "r"(r0), "r"(r1) : "memory");
}

/* { dg-final { scan-rtl-dump-times {(?:;;\s+1--> b\s+\d+: i\s+\d+ \S+=\S+\*\S+[^\n]*\n;;\s+1--> b\s+\d+: i\s+\d+ \S+=\S+\+[a-z]+[0-9]|;;\s+2--> b\s+\d+: i\s+\d+ \S+=\S+\*\S+[^\n]*\n;;\s+2--> b\s+\d+: i\s+\d+ \S+=\S+\+[a-z]+[0-9]|;;\s+3--> b\s+\d+: i\s+\d+ \S+=\S+\*\S+[^\n]*\n;;\s+3--> b\s+\d+: i\s+\d+ \S+=\S+\+[a-z]+[0-9]|;;\s+4--> b\s+\d+: i\s+\d+ \S+=\S+\*\S+[^\n]*\n;;\s+4--> b\s+\d+: i\s+\d+ \S+=\S+\+[a-z]+[0-9]|;;\s+5--> b\s+\d+: i\s+\d+ \S+=\S+\*\S+[^\n]*\n;;\s+5--> b\s+\d+: i\s+\d+ \S+=\S+\+[a-z]+[0-9]|;;\s+6--> b\s+\d+: i\s+\d+ \S+=\S+\*\S+[^\n]*\n;;\s+6--> b\s+\d+: i\s+\d+ \S+=\S+\+[a-z]+[0-9])} 2 "sched2" } } */
