/* Verify that the RHX-100 scheduler attributes every instruction of a
   ...
   Group under test:
     M (unfused memory op) + M (unfused memory op) + M (unfused memory op) + M (unfused memory op)
   ...
   which is a multiply unit.  This group must issue in 4 cycles.
...
f_m_m_m_m (void)
{
  ...
  asm volatile ("" : "=r"(p0), "=r"(p1), "=r"(p2), "=r"(p3));
  int r0 = p0[64];
  int r1 = p1[128];
  int r2 = p2[192];
  int r3 = p3[256];
...
/* { dg-final { scan-rtl-dump {;;\s+4--> b\s+\d+: i\s+\d+ \S+=\[\S+\+0x[1-4]00\]} "sched2" } } */
